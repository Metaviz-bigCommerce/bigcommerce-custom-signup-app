...
import { resubmissionRequestSchema, requestIdSchema } from '@/lib/validation';
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const validation = resubmissionRequestSchema.safeParse(body);
		...
		const { problematicFields, message } = validation.data;
		...
		// Update request for resubmission
		await (db as any).updateSignupRequestForResubmission(
			storeHash,
			id,
			problematicFields,
			message
		...
		// Format problematic fields for email
		const problematicFieldsText = problematicFields.join(', ');
		...
		if (email) {
			try {
				const emailResult = await trySendTemplatedEmail({
					to: email,
					template: templates.moreInfo, // Resubmission request template
					vars: {
						name,
						email: email || '',
						...
						required_information: problematicFieldsText,
			...
			logger.warn('Cannot send resubmission request email: no email address', { ...logContext, storeHash, requestId: id });
		}
		
		return successResponse({ sent: true }, 200, requestId);
	} catch (error: unknown) {
		logger.error('Failed to send resubmission request', error, logContext);
		return apiErrors.internalError('Failed to send resubmission request', error, requestId);
	}
}
