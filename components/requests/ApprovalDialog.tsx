    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-3 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100">
          <div className="text-base sm:text-lg font-bold text-gray-900">Approve request</div>
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-5 flex-1 overflow-y-auto">
          {groupsLoading ? (
            ...
            <>
              <div className="text-xs sm:text-sm font-medium text-gray-800 mb-2">Customer group</div>
              <div className="space-y-2 max-h-64 overflow-auto border border-gray-200 rounded-md p-2 sm:p-3">
                <label className="flex items-center gap-2 text-xs sm:text-sm text-gray-800 cursor-pointer hover:bg-gray-50 p-1.5 rounded-md transition-colors">
                  <input
                    type="radio"
                    name="customer_group"
                    checked={selectedGroupId === null}
                    onChange={() => setSelectedGroupId(null)}
                    className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-600 focus:ring-emerald-500"
                  />
                ...
                {customerGroups.map((g) => (
                  <label 
                    key={g.id} 
                    className="flex items-center gap-2 text-xs sm:text-sm text-gray-800 cursor-pointer hover:bg-gray-50 p-1.5 rounded-md transition-colors"
                  >
                    <input
                      type="radio"
                      name="customer_group"
                    ...
                    <span className="flex-1">
                      ...
                      {g.is_default && (
                        <span className="ml-2 text-[10px] sm:text-xs text-gray-500">(default)</span>
                      )}
                    </span>
                  </label>
                ))}
                {!customerGroups.length && (
                  <div className="text-xs sm:text-sm text-gray-500 p-2">No customer groups found.</div>
                )}
              </div>
            </>
          )}
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-100 bg-gray-50 flex flex-col sm:flex-row gap-2 sm:gap-3">
          <button
            onClick={handleClose}
            className="w-full sm:w-auto px-4 py-2 sm:py-2.5 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-100 transition-colors font-medium text-xs sm:text-sm cursor-pointer"
            disabled={approving}
          >
            Cancel
          </button>
          <button
            ...
            disabled={approving}
            className="flex-1 px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg bg-emerald-600 text-white text-xs sm:text-sm font-semibold hover:bg-emerald-700 active:bg-emerald-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2 cursor-pointer"
          >
            {approving ? (
              <>
              ...
              <>
                ...
                <span className="hidden sm:inline">Approve and Create Customer</span>
